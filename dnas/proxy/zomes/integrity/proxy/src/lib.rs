pub mod proxied_role;
use hdi::prelude::*;
pub use proxied_role::*;
                ...
                match app_entry {
                    EntryTypes::ProxiedRole(proxied_role) => {
                        ...
                        let original_proxied_role = match ProxiedRole::try_from(original_app_entry)
                        {
                            ...
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get ProxiedRole from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_proxied_role(
                            action,
                            proxied_role,
                    ...
                    match app_entry {
                        EntryTypes::ProxiedRole(proxied_role) => {
                            ...
                            if let ValidateCallbackResult::Valid = result {
                                ...
                                let original_proxied_role = match original_proxied_role {
                                    Some(proxied_role) => proxied_role,
                                    None => {
                                        return Ok(
