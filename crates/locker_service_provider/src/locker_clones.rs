...
pub async fn reconcile_locker_clones(
    ...
    let locker_cells = app_info
        .cell_info
        .get("locker")
        .cloned()
        .unwrap_or_default();

    let cloned_cells: Vec<ClonedCell> = locker_cells
        .iter()
    ...
    log::info!("New epoch time reached: deleting the current locker cell if it exists and creating a new one.");
    ...
    let cloned_cell = app_ws
        .create_clone_cell(CreateCloneCellPayload {
            role_name: RoleName::from("locker"),
    ...
    if let Some(previous_cell) = previous_cell {
        ...
        log::info!("Deleting the locker clone cell for the current epoch.");
        app_ws
    ...
    log::info!(
        "Successfully advanced locker clone epoch, new clone: {}",
